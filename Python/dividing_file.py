...
def main():
    for k in [0.5]:
        for iterr in [1]:
            ...
            demod_sym_stack = []
            Peaks = []
            for m in range(8, 11):#range(idx.shape[0]):
                print('Packet ' + str(m+1) + '/' + str(idx.shape[0]))
                ##      DC correlations
                temp_buff = []
                temp_buff = x_1[int(round(idx[m, 0])) : int(idx[m, 1]) + 1]
                print('samples: ', len(temp_buff))
                st = time.perf_counter()
                # NOTE: Roughly 60% of the time is spent here
                ind = DC_location_correlation(temp_buff[::upsampling_factor], N, DC[:N], 16, 0.2) # 19/26 seconds
                print('dcloccorrtime: ', time.perf_counter() - st)
                if (ind.shape[0] == 0):
                    continue
                ...
                for o in range(Rx_Buffer.shape[0]):
                    ...
                    # print('uccorrtime: ', time.perf_counter() - stt)
                    # print('Upchirp_ind got: ', Upchirp_ind)
                    if (Upchirp_ind.shape[0] == 0):
                        continue
                    # stt = time.perf_counter()
                    # NOTE: downsample takes about 20% of time
                    [Data_freq_off, Peak, Upchirp_ind] = dnsamp_buff(Rx_Buff_dnsamp, Upchirp_ind, num_preamble, num_sync, num_DC, N, DC_pre)
                    # print('dnsamptime: ', time.perf_counter() - stt)
                    if (Upchirp_ind.shape[0] == 0):
                        continue
                    # stt = time.perf_counter()
                    ...
                    # print('ffptime: ', time.perf_counter() - stt)
                    # stt = time.perf_counter()
                    temp = []
                    temp_data = []
                    temp_peaks = []
                    ...
                    demod_sym = []
                    sym = 0
                    # print('beforedemod: ', time.perf_counter() - stt)
